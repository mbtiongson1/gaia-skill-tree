...
- [ ] Seed `registry/gaia.json` with:
  ...
  - 2–3 legendary stubs (clearly `provisional`, Level 1⭐ only).
...
- [ ] Implement `scripts/validate.py`:
  ...
  - Evidence threshold by level (Level 2⭐ needs Evidence Tier C+, Level 3⭐ needs Evidence Tier B+, etc.).
...
- [ ] CI correctly catches: a cycle, a missing parent reference, a Level 3⭐ skill with no Evidence Tier B source.
...
### Legendary Stubs (stubs only at launch, Level 1⭐, provisional)
```
...
- % of Level 3⭐+ skills with at least one Evidence Tier B source.
