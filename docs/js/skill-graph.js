(function () {
  ...
  const NODE_RADII = {
    ...
    get: function (rank, type) {
      if (type === 'unique') return this.unique;  // use static 9.5 (fix: issue #377)
  ...
  function getCanvasTokens() {
    ...
    _tokenCache = {
      ...
      clusterRgb: [0, 1, 2, 3, 4, 5, 6, 7].map(n => _rgbOnly(_readVar('--cluster-' + n + '-rgb'))),
  ...
  function createSkillGraph(canvas, options) {
    ...
    function draw() {
      ...
      edges.forEach(edge => {
        ...
        // Higher Z is further away, so we want lower alpha as Z increases
        const depthAlpha = Math.min(Math.max(1 - (xf[edge.to].z / (860 * state.scale)), 0.08), 1);
      ...
      nodes.forEach(({ skill }) => {
        ...
        // Z > 0 is far away, so we fade as Z increases. W can be positive or negative.
        const depthAlpha = Math.min(1, Math.max(0.18, (1 - pr.z / 650) * (1 - Math.abs(pr.w) / 600)));

        let col;
        if (state.colorMode === 'cluster' && skill.cluster !== undefined) {
          const clusterRgb = getCanvasTokens().clusterRgb;
          const rgb = clusterRgb[skill.cluster % clusterRgb.length] || getCanvasTokens().tier.basic.rgb;
          col = { rgb, hex: '' };
        } else {
      ...
      function drawLabel(skill, highlighted, forcedDepthAlpha, forcedColRgb) {
        const p = xf[skill.id]; if (!p) return;
        const pr = project(p);
        const depthAlpha = forcedDepthAlpha !== undefined ? forcedDepthAlpha : Math.min(Math.max(1 - (p.z / (860 * state.scale)), 0), 1);
  ...
  fetch(GRAPH_JSON_URL, { cache: 'reload' })
    .then(response => {
      ...
      // Reject text/html explicitly: Workers may serve an HTML 404 page whose
      // content-type slips through the old !includes('json') guard, causing
      // response.json() to throw silently → FALLBACK_SKILLS (issue #377).
      if (ct.includes('text/html') || (!ct.includes('json') && !ct.includes('text/plain'))) {
        return response.text().then(body => {
          throw new Error(`Expected JSON, got ${ct || 'no content-type'}; body starts: ${body.slice(0, 120)}`);
        });
      }
